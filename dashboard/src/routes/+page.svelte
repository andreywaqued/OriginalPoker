<script>
    ...
    async function updateUsers() {
        ...
        // users = [{username: "asd"}]
        // return users
        const response = await fetch('http://originaltrial:10000/getUsers')
        if (response.ok) {
    ...
    async function updateHands() {
        ...
        // users = [{username: "asd"}]
        // return users
        const response = await fetch('http://originaltrial:10000/getHandsCount')
        if (response.ok) {
            const data = await response.json()
            handsPlayed = data
        } else {
    ...
    async function sendChips(index, userid) {
        ...
        if (chipsValue) {
            const response = await fetch(`http://originaltrial:10000/addchips?user=${userid}&chips=${chipsValue}`)
            if (response.ok) {
                updateUsers()
            } else {
