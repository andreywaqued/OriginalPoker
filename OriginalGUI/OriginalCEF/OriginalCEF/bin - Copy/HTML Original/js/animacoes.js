...
function fUpdatePlayerBet(sitPosition, betAmount) {
    ...
    console.log("muda digito vindo de fUpdatePlayerBet")
...
function fBtnBetPerc(perc) {
    ...
    for (i = 0; i < globalTableSize; i++) {
        totalPot += conjunto_jogadores[i].valor.betAmount; //parseFloat(document.getElementById(`betAmount${i}`).innerHTML.substring(1, document.getElementById(`betAmount${i}`).innerHTML.length));
        if (i > 0) {
            if (conjunto_jogadores[i].valor.betAmount > currentBet) {
                currentBet = conjunto_jogadores[i].valor.betAmount;
            }
        }
    }

    for (i = 0; i < totalPots - 1; i++) {
        totalPot += parseFloat(document.getElementById("pot" + i + "Amount").innerHTML);
    }
    console.log(`fBtnBetPerc(${perc}) - totalPot:${totalPot}`);
    console.log(`fBtnBetPerc(${perc}) - currentBet:${currentBet}`);
    console.log(`fBtnBetPerc(${perc}) - globalAmountToCall:${globalAmountToCall}`);
