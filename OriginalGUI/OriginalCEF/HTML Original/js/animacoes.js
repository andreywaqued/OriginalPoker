...
digitos_por_jogador = [];
limite_digitos = { 'stack': { 'maximo': 16, 'minimo': 1, 'atual': 16 }, 'betAmount': { 'maximo': 16, 'minimo': 1, 'atual': 16 } };
...
texto = {
    ...
    'betAmount': {
        'dimensoes': { 'largura': (limite_digitos.betAmount.maximo + 4) * (1.05 / 2) * 12 },
...
parametros_animacao = {
    ...
    'anima_betAmount': { 'limite_betAmount': 10, 'distancia_entre_betAmount': 4, 'inicio_queda': -50, 'velocidade': 0.1 },
    'anima_betAmount2': [{
            'posicao': 40,
            'velocidade': 1
        },
...
conjunto_jogadores = [{
            ...
            'posicao_betAmount': { 'x': mesa.largura_total * 0.58, 'y': mesa.altura_total * 0.65 - Math.round(betAmount.dimensoes.altura / 2) },
            'valor': { 'stack': 0, 'betAmount': 0 },
            'direcao': { 'stack': 0, 'betAmount': 0 },
        ...
        },
        {
            ...
            'posicao_betAmount': { 'x': mesa.largura_total * 0.30, 'y': mesa.altura_total * 0.60 - Math.round(betAmount.dimensoes.altura / 2) },
            'valor': { 'stack': 0, 'betAmount': 0 },
            'direcao': { 'stack': 0, 'betAmount': 0 },
        ...
        },
        {
            ...
            'posicao_betAmount': { 'x': mesa.largura_total * 0.32, 'y': mesa.altura_total * 0.32 - Math.round(betAmount.dimensoes.altura / 2) },
            'valor': { 'stack': 0, 'betAmount': 0 },
            'direcao': { 'stack': 0, 'betAmount': 0 },
            'anima_principal': { 'rodando': false, 'tipo_movimento': '' }
        },
        {
            ...
            'posicao_betAmount': { 'x': mesa.largura_total * 0.50, 'y': mesa.altura_total * 0.30 - Math.round(betAmount.dimensoes.altura / 2) },
            'valor': { 'stack': 0, 'betAmount': 0 },
            'direcao': { 'stack': 0, 'betAmount': 0 },
            'anima_principal': { 'rodando': false, 'tipo_movimento': '' }
        },
        {
            ...
            'posicao_betAmount': { 'x': mesa.largura_total * 0.68, 'y': mesa.altura_total * 0.35 - Math.round(betAmount.dimensoes.altura / 2) },
            'valor': { 'stack': 0, 'betAmount': 0 },
            'direcao': { 'stack': 0, 'betAmount': 0 },
            'anima_principal': { 'rodando': false, 'tipo_movimento': '' }
        },
        {
            ...
            'posicao_betAmount': { 'x': mesa.largura_total * 0.70, 'y': mesa.altura_total * 0.58 - Math.round(betAmount.dimensoes.altura / 2) },
            'valor': { 'stack': 0, 'betAmount': 0 },
            'direcao': { 'stack': 0, 'betAmount': 0 },
            'anima_principal': { 'rodando': false, 'tipo_movimento': '' }
...
function init() {
    ...
    /*for (sitPosition = 0; sitPosition < conjunto_jogadores.length; sitPosition++) {
        ...
        html = html + "<div id='caixa_betAmount_jogador" + sitPosition + "' style='position:absolute;display:none;opacity:0;overflow-y:hidden;overflow-x:hidden;width:" + betAmount.caixa_betAmount.largura + "px;height:" + betAmount.dimensoes.altura + "px;margin-top:" + conjunto_jogadores[sitPosition].posicao_betAmount.y + "px;margin-left:" + (conjunto_jogadores[sitPosition].posicao_betAmount.x - (betAmount.caixa_betAmount.largura / 2)) + "px'></div>";
        html = html + "<div style='position:absolute;z-index:4;width:" + texto.betAmount.dimensoes.largura + "px;margin-left:" + (conjunto_jogadores[sitPosition].posicao_betAmount.x - texto.betAmount.dimensoes.largura / 2) + "px;'><div id='betAmount" + sitPosition + "' style='display:block;overflow-y:hidden;overflow-x:hidden;float:right;height:" + texto.betAmount.digito.altura + "px;background-color:gray;color:white;opacity:0%;border:3px solid gray;border-radius:3px;font-size:" + texto.betAmount.digito.altura + "px;text-align:right;margin-top:" + (conjunto_jogadores[sitPosition].posicao_betAmount.y * 1 + (betAmount.dimensoes.altura / 2) - texto.betAmount.digito.altura) + "px;z-index:4'><div id='totalbetAmount" + sitPosition + "' style='float:right'></div></div></div>";
        ...
        //html = html + "<div id='marca_posicao_betAmount" + sitPosition + "' style='position:absolute;background-color:blue;z-index:20;height:3px;width:3px;margin-left:" + conjunto_jogadores[sitPosition].posicao_betAmount.x + "px;margin-top:" + conjunto_jogadores[sitPosition].posicao_betAmount.y + "px'></div></div>";

        digitos_por_jogador[sitPosition] = { 'pot': { 'maximo': 0, 'minimo': 0, 'atual': 0 }, 'betAmount': { 'maximo': 0, 'minimo': 0, 'atual': 0 } };
        ...
        inicializa_digitos_valores(sitPosition, 'betAmount');
...
function fInitGameTest() {
    fSitHero(0, "ZeeJustin", 1); fUpdatePlayerStack(0,"230.32");
    window.setTimeout(function() { fSitPlayer(1, "DanPolksa", 2); fUpdatePlayerStack(1,200); }, 200);    
    window.setTimeout(function() { fSitPlayer(2, "1_conor_b_1", 3); fUpdatePlayerStack(2,200);}, 400);
    window.setTimeout(function() { fSitPlayer(3, "KissSS55S", 4); fUpdatePlayerStack(3,115.58);}, 600);
    window.setTimeout(function() { fSitPlayer(4, "colisea", 5); fUpdatePlayerStack(4,68.30);}, 800);
    window.setTimeout(function() { fSitPlayer(5, "lechuckpoker", 6); fUpdatePlayerStack(5,439);}, 1000);


    window.setTimeout(function() { fUpdatePlayerBet(1,1); fUpdatePlayerStack(1,199);},1200);
    window.setTimeout(function() { fUpdatePlayerBet(2,2); fUpdatePlayerStack(2,198);},1400);
    
    window.setTimeout(function() {fAddCardToSlot('card1', 0, "9s");fAddCardToSlot('card2', 0, "Js");},1600);
    window.setTimeout(function() {fAddCardToSlot('card1', 1, "card");fAddCardToSlot('card2', 1, "card");},1800);
    window.setTimeout(function() {fAddCardToSlot('card1', 2, "card");fAddCardToSlot('card2', 2, "card");},2000);
    window.setTimeout(function() {fAddCardToSlot('card1', 3, "card");fAddCardToSlot('card2', 3, "card");},2200);
    window.setTimeout(function() {fAddCardToSlot('card1', 4, "card");fAddCardToSlot('card2', 4, "card");},2400);
    window.setTimeout(function() {fAddCardToSlot('card1', 5, "card");fAddCardToSlot('card2', 5, "card");},2600);


    window.setTimeout(function() {fUpdatePlayerTurn(3, '30.00');},2600);
    window.setTimeout(function() {fFoldPlayerCards(3);},7600);

    window.setTimeout(function() {fUpdatePlayerTurn(4, '30.00');},7600);
    window.setTimeout(function() {fFoldPlayerCards(4);},10000);
 
    window.setTimeout(function() {fUpdatePlayerTurn(5, '30.00');},10000);
    window.setTimeout(function() {fFoldPlayerCards(5);},10100);

    window.setTimeout(function() {fLocalPlayerTurn('2.00', '4.00', '201.00', ['FOLD', 'CALL', 'RAISE'], '30.00');},10100);
    window.setTimeout(function() {fFoldPlayerCards(5);},14100);
    window.setTimeout(function() {fDisableActionButtons();},14100);
        
    window.setTimeout(function() {fResetTable();},14200);


    window.setTimeout(function() { fSitHero(0, "ZeeJustin", 1); fUpdatePlayerStack(0,"230.32");},15000);
    window.setTimeout(function() { fSitPlayer(1, "DanPolksa", 2); fUpdatePlayerStack(1,200);}, 15200);    
    window.setTimeout(function() { fSitPlayer(2, "1_conor_b_1", 3); fUpdatePlayerStack(2,200);}, 15400);
    window.setTimeout(function() { fSitPlayer(3, "KissSS55S", 4); fUpdatePlayerStack(3,115.58);}, 15600);
    window.setTimeout(function() { fSitPlayer(4, "colisea", 5); fUpdatePlayerStack(4,68.30);}, 15800);
    window.setTimeout(function() { fSitPlayer(5, "lechuckpoker", 6); fUpdatePlayerStack(5,439);}, 151000);


    window.setTimeout(function() { fUpdatePlayerBet(1,1); fUpdatePlayerStack(1,199);},16200);
    window.setTimeout(function() { fUpdatePlayerBet(2,2); fUpdatePlayerStack(2,198);},16400);
    
    window.setTimeout(function() {fAddCardToSlot('card1', 0, "9s");fAddCardToSlot('card2', 0, "Js");},16600);
    window.setTimeout(function() {fAddCardToSlot('card1', 1, "card");fAddCardToSlot('card2', 1, "card");},16800);
    window.setTimeout(function() {fAddCardToSlot('card1', 2, "card");fAddCardToSlot('card2', 2, "card");},17000);
    window.setTimeout(function() {fAddCardToSlot('card1', 3, "card");fAddCardToSlot('card2', 3, "card");},17200);
    window.setTimeout(function() {fAddCardToSlot('card1', 4, "card");fAddCardToSlot('card2', 4, "card");},17400);
    window.setTimeout(function() {fAddCardToSlot('card1', 5, "card");fAddCardToSlot('card2', 5, "card");},17600);


    window.setTimeout(function() {fUpdatePlayerTurn(3, '30.00');},17600);
    window.setTimeout(function() {fFoldPlayerCards(3);},18600);

    window.setTimeout(function() {fUpdatePlayerTurn(4, '30.00');},18600);
    window.setTimeout(function() {fFoldPlayerCards(4);},20000);
 
    window.setTimeout(function() {fUpdatePlayerTurn(5, '30.00');},20000);
    window.setTimeout(function() {fFoldPlayerCards(5);},20100);

    window.setTimeout(function() {fLocalPlayerTurn('2.00', '4.00', '201.00', ['FOLD', 'CALL', 'RAISE'], '30.00');},20100);
    window.setTimeout(function() {fUpdatePlayerBet(0, 4.00); fUpdatePlayerStack(0,226.32);},23100);
    window.setTimeout(function() {fDisableActionButtons();},23100);
        
    window.setTimeout(function() {fUpdatePlayerTurn(1, '30.00');},23100);
    window.setTimeout(function() {fFoldPlayerCards(1);},24100);

    window.setTimeout(function() {fUpdatePlayerTurn(2, '30.00');},24100);
    window.setTimeout(function() {fUpdatePlayerBet(2, 4.00); fUpdatePlayerStack(0,196.00);},25100);
    window.setTimeout(function() {fDisableTime(2);},25100);

    window.setTimeout(function() {fUpdatePotAmount(0, 9.00);fUpdatePlayerStack(0,0);fUpdatePlayerStack(1,0);fUpdatePlayerStack(2,0);},25100);
    window.setTimeout(function() {fAddCardToBoard(boardC1, '8s');fAddCardToBoard(boardC2, '9d');fAddCardToBoard(boardC3, 'Qc');},25100);

    window.setTimeout(function() {fUpdatePlayerTurn(2, '30.00');},25100);
    window.setTimeout(function() {fUpdatePlayerBet(2, 4.00); fUpdatePlayerStack(0, 0);},27100);
    window.setTimeout(function() {fDisableTime(2);},27100);

    window.setTimeout(function() {fLocalPlayerTurn('2.00', '4.00', '201.00', ['FOLD', 'CALL'], '30.00');},27100);
    window.setTimeout(function() {fUpdatePlayerBet(0, 196.00); fUpdatePlayerStack(0,30.32);},30100);
    window.setTimeout(function() {fDisableActionButtons();},30100);

    window.setTimeout(function() {fUpdatePotAmount(0, 401.00);fUpdatePlayerStack(0,0);fUpdatePlayerStack(1,0);fUpdatePlayerStack(2,0);},30100);


    window.setTimeout(function() {fAddCardToSlot('card1', 2, "Ts");fAddCardToSlot('card2', 2, "Qh");},30200);
    window.setTimeout(function() {fAddCardToBoard(boardC4, 'As');},31200);
    window.setTimeout(function() {fAddCardToBoard(boardC5, 'Ks');},31400);

    window.setTimeout(function() {fShowWinner(0,401.00);},32400);
    window.setTimeout(function() {fUpdatePlayerStack(0,431.32);},30100);

    window.setTimeout(function() {fResetTable();},15200);
    ...
    if (Sit == -1) {
        for (i = 0; i <= globalTableSize - 1; i++) {
            ...
            if (!reservedSits.includes(i)) {
                ...
                if (document.getElementById(`seat${sitPosition}`) != null) {
...
function fDisableActionButtons() {
    try {
        ...
        if (document.getElementById(`timerSeat0`) != null){
...
function fSitPlayer(sitPosition, playerName, idAvatar) {
    ...
    try {
        ...
        html = html + "<div style='position:absolute;z-index:8;width:" + (1.4 * jogador_geral.largura_total * conjunto_jogadores[sitPosition].proporcao - 4) + "px;margin-left:" + (margin_left * 1 - 0.2 * jogador_geral.largura_total * conjunto_jogadores[sitPosition].proporcao + 2) + "px;margin-top:" + (margin_top * 1 - texto.valor_jogador.dimensoes.altura * 0.8 * conjunto_jogadores[sitPosition].proporcao) + "px;'><div id='campostack" + sitPosition + "' style='display:block;overflow-y:hidden;overflow-x:hidden;float:right;height:" + texto.valor_jogador.digito.altura * conjunto_jogadores[sitPosition].proporcao + "px;color:white;font-size:" + texto.valor_jogador.digito.altura * conjunto_jogadores[sitPosition].proporcao + "px;text-align:right;padding:0px 2px 0px 0px;margin-top:" + 1.04 * texto.valor_jogador.posicao.y * conjunto_jogadores[sitPosition].proporcao + "px;'><div id='totalstack" + sitPosition + "' style=''></div></div></div>";
        ...
        digitos_por_jogador[sitPosition] = { 'stack': { 'maximo': 0, 'minimo': 0, 'atual': 0 }, 'betAmount': { 'maximo': 0, 'minimo': 0, 'atual': 0 } };
        inicializa_digitos_max_min(sitPosition, 'todos');

        //console.log("html da mesa: " + document.getElementById('mesa').innerHTML)
        document.getElementById('mesa').innerHTML = document.getElementById('mesa').innerHTML + html;
        inicializa_digitos_valores(sitPosition, 'betAmount');
        inicializa_digitos_valores(sitPosition, 'stack');
    } catch (err) {
        console.log(`Error on fSitPlayer(${sitPosition}, ${playerName}): ${err}`);
    }
}
...
function fUpdatePlayerBet(sitPosition, betAmount) {
    ...
    if (betAmount == 0) {
        ...
        console.log("chamando anima_opacity betAmount = 0")
    ...
    document.getElementById(`betAmount${sitPosition}`).style.display = 'block'
    //document.getElementById(`caixa_betAmount_jogador${sitPosition}`).style.display = "block"
    //document.getElementById(`betAmount${sitPosition}`).style.overflowY = "visible"
    //document.getElementById(`caixa_betAmount_jogador${sitPosition}`).style.overflowY = "visible"
    valor_inicial = conjunto_jogadores[sitPosition].valor.betAmount;
    conjunto_jogadores[sitPosition].valor.betAmount = betAmount;
    //document.getElementById(`betAmount${sitPosition}`).style.overflow-y = "visible";
    //document.getElementById(`betAmount${sitPosition}`).style.overflow-x = "visible";
    console.log("Jogador " + sitPosition + " betAmount " + conjunto_jogadores[sitPosition].valor.betAmount + " visibilidade " + document.getElementById(`betAmount${sitPosition}`).style.display);
    muda_digito("betAmount", sitPosition, betAmount, valor_inicial)
}
...
function fUpdatePlayerStack(sitPosition, stackAmount) {
    ...
    valor_inicial = conjunto_jogadores[sitPosition].valor.stack;
    conjunto_jogadores[sitPosition].valor.stack = stackAmount;
    //console.log("muda digito vindo de fUpdatePlayerStack")
    muda_digito("stack", sitPosition, stackAmount, valor_inicial)
...
function fBtnBetPerc(perc) {
    ...
    for (i = 0; i < globalTableSize; i++) {
        console.log(`adicionando valor ${conjunto_jogadores[i].valor.betAmount} do jogador ${i}`)
        totalPot += parseFloat(conjunto_jogadores[i].valor.betAmount); //parseFloat(document.getElementById(`betAmount${i}`).innerHTML.substring(1, document.getElementById(`betAmount${i}`).innerHTML.length));
        if (i > 0) {
            if (parseFloat(conjunto_jogadores[i].valor.betAmount) > currentBet) {
                currentBet = parseFloat(conjunto_jogadores[i].valor.betAmount);
            }
        }
    }

    for (i = 0; i < totalPots - 1; i++) {
        console.log(`pot ${i} ${document.getElementById("pot" + i + "Amount").innerHTML.substring(1, document.getElementById("pot" + i + "Amount").innerHTML.length)}`)
        totalPot += parseFloat(document.getElementById("pot" + i + "Amount").innerHTML.substring(1, document.getElementById("pot" + i + "Amount").innerHTML.length));
    }
    console.log(`fBtnBetPerc(${perc}) - totalPot:${totalPot}`);
    console.log(`fBtnBetPerc(${perc}) - currentBet:${currentBet}`);
    console.log(`fBtnBetPerc(${perc}) - globalAmountToCall:${globalAmountToCall}`);
    //console.log(`fBtnBetPerc(${perc}) - betAmount0:${parseFloat(betAmount0.innerHTML.substring(1, betAmount0.innerHTML.length))}`);
    console.log(`fBtnBetPerc(${perc}) - betAmount0:${parseFloat(conjunto_jogadores[0].valor.betAmount.substring(1, conjunto_jogadores[0].valor.betAmount.length))}`);
    ...
    raiseSize = parseFloat(currentBet) + ((parseFloat(totalPot) + parseFloat(currentBet) - parseFloat(conjunto_jogadores[0].valor.betAmount.substring(1, conjunto_jogadores[0].valor.betAmount.length))) * parseFloat(perc) / 100)
...
function proxima_animacao_demo(velocidade) {
    switch (demo_animacao_atual) {
        ...
        case 3:
            ...
            break;
        case 4:
            alert("Demo anima betAmount - Valor entrada:1.262 - 2 betAmount de cada em 5");
            demo_anima_betAmount(velocidade);
            break;
        case 5:
            alert("Demo betAmount -> centro");
            demo_betAmount_centro(velocidade);
            break;
        case 6:
...
function demo_primeira_vez(velocidade) {
    ...
    if (variavel_jogador < conjunto_jogadores.length - limitante_jogadores_demo) {
        ...
        valor_atual = conjunto_jogadores[variavel_jogador].valor['stack'];
        valor_final = 9000000;
        conjunto_jogadores[variavel_jogador].valor['stack'] = valor_final;
        muda_digito('stack', variavel_jogador, valor_final, valor_atual);

        valor_atual = conjunto_jogadores[variavel_jogador].valor['betAmount'];
        valor_final = 200870543;
        conjunto_jogadores[variavel_jogador].valor['betAmount'] = valor_final;
        muda_digito('betAmount', variavel_jogador, valor_final, valor_atual);
...
function demo_operacoes_adicao(velocidade) {
    ...
    if (variavel_jogador < conjunto_jogadores.length - limitante_jogadores_demo) {
        ...
        operacao(variavel_jogador, 'stack', 'adicao', 9001234);

        operacao(variavel_jogador, 'betAmount', 'subtracao', 9001234);
...
function demo_operacoes_subtracao(velocidade) {
    ...
    if (variavel_jogador < conjunto_jogadores.length - limitante_jogadores_demo) {
        ...
        operacao(variavel_jogador, 'stack', 'subtracao', 8906789);

        operacao(variavel_jogador, 'betAmount', 'adicao', 8906789);
...
function demo_anima_betAmount(velocidade) {

    if (variavel_jogador < conjunto_jogadores.length - limitante_jogadores_demo) {
        anima_opacity('betAmount' + variavel_jogador, 'diminui', '0.0', 0.8)

        //grupos_betAmount = valor_em_betAmount(conjunto_jogadores[variavel_jogador].valor['betAmount'])
        grupos_betAmount = valor_em_betAmount(1262);
        anima_betAmount(variavel_jogador, grupos_betAmount);
        setTimeout(function() {
            variavel_jogador++;
            demo_anima_betAmount(velocidade);
        }, velocidade);
...
function demo_betAmount_centro(velocidade) {

    if (variavel_jogador < conjunto_jogadores.length - limitante_jogadores_demo) {
        anima_principal(variavel_jogador, 'betAmount->centro');
        setTimeout(function() {
            variavel_jogador++;
            demo_betAmount_centro(velocidade)
        }, velocidade);
 ...
 * Objetivo: animações de cartas e betAmount
...
function anima_principal(sitPosition, tipo_movimento, cardSlot = '') {
    ...
    if ((conjunto_jogadores[sitPosition].anima_principal.tipo_movimento == '') || (conjunto_jogadores[sitPosition].anima_principal.tipo_movimento == tipo_movimento) || !conjunto_jogadores[sitPosition].anima_principal.rodando) {
        ...
        //Movimento de betAmount ao centro
        if (tipo_movimento == 'betAmount->centro') {

            //Define ponto de destino das betAmount nas duas coordenadas
            centro_horizontal = mesa.largura_total * mesa.centro.x;
            centro_vertical = mesa.altura_total * mesa.centro.y + (betAmount.dimensoes.altura / 2);
            ...
            //Define ponto de origem das betAmount nas duas coordenadas
            margin_left = conjunto_jogadores[sitPosition].posicao_betAmount.x;
            margin_top = conjunto_jogadores[sitPosition].posicao_betAmount.y;


            //Definições dos parâmetros a e b da equação da reta y=a*x+b por onde as betAmount irão percorrer
            a = (margin_top - centro_vertical) / (margin_left - centro_horizontal);
            b = margin_top - a * margin_left;
            anima = true;


            //O passo da animação de caminhada sobre a equação da reta deverá ser o mesmo para todas as betAmount, independentemente do ângulo. Porém, como y é definido em função de x,
            ...
            //Se as betAmount estiverem à direita do centro.


            if ((document.getElementById('caixa_betAmount_jogador' + sitPosition).style.marginLeft.replace('px', '') * 1 + Math.round(betAmount.caixa_betAmount.largura / 2)) > (centro_horizontal + margem_centro)) {

                //Bloco movimento
                document.getElementById('caixa_betAmount_jogador' + sitPosition).style.marginLeft = (document.getElementById('caixa_betAmount_jogador' + sitPosition).style.marginLeft.replace('px', '') * 1 - step_x) + 'px';
                x = (document.getElementById('caixa_betAmount_jogador' + sitPosition).style.marginLeft.replace('px', '') * 1) + Math.round(betAmount.caixa_betAmount.largura / 2);
                y = a * x + b;

                document.getElementById('caixa_betAmount_jogador' + sitPosition).style.marginTop = y + 'px';
                ...
                taxa_reducao = (document.getElementById('caixa_betAmount_jogador' + sitPosition).style.marginLeft.replace('px', '') * 1 + Math.round(betAmount.caixa_betAmount.largura / 2) - centro_horizontal) / (margin_left - centro_horizontal);
                ...
                document.getElementById('caixa_betAmount_jogador' + sitPosition).style.opacity = (carta.opacidade_ao_centro.minimo + (carta.opacidade_ao_centro.maximo - carta.opacidade_ao_centro.minimo) * taxa_reducao);
            }
            //Se as betAmount estiverem à esquerda do centro.
            else if ((document.getElementById('caixa_betAmount_jogador' + sitPosition).style.marginLeft.replace('px', '') * 1 + Math.round(betAmount.caixa_betAmount.largura / 2)) < (centro_horizontal - margem_centro)) {
            ...
            //Para os casos em que as betAmount não possuem deslocamento em x e estão localizadas abaixo do centro
            else if ((document.getElementById('caixa_betAmount_jogador' + sitPosition).style.marginTop.replace('px', '') * 1) > (centro_vertical + margem_centro)) {

                //Bloco movimento
                step = (parametros_animacao.anima_carta.movimento_carta_velocidade);
                document.getElementById('caixa_betAmount_jogador' + sitPosition).style.marginTop = ((document.getElementById('caixa_betAmount_jogador' + sitPosition).style.marginTop.replace('px', '') * 1) - step) + 'px';

                //A taxa de redução é um valor que varia entre 0 e 1 da origem para o destino, pode ser usado para controlar escala ou opacidade                                    
                taxa_reducao = (document.getElementById('caixa_betAmount_jogador' + sitPosition).style.marginTop.replace('px', '') * 1 - centro_vertical) / (margin_top - centro_vertical);

                //Variação de opacidade, de acordo com a caminhada origem->destino. Parâmetros máxmo e mínimo controlados nas variáveis globais.                       
                document.getElementById('caixa_betAmount_jogador' + sitPosition).style.opacity = (carta.opacidade_ao_centro.minimo + (carta.opacidade_ao_centro.maximo - carta.opacidade_ao_centro.minimo) * taxa_reducao);
            }
            //Para os casos em que as betAmount não possuem deslocamento em x e estão localizadas acima do centro
            else if ((document.getElementById('caixa_betAmount_jogador' + sitPosition).style.marginTop.replace('px', '') * 1) < (centro_vertical - margem_centro)) {
...
/**
 * Função: OPERAÇÃO
 * Objetivo: realizar operações matemáticas nos elementos pot e betAmount.
 * Como:
...
function operacao(sitPosition, betAmount_ou_pot, operacao, valor_operacao) {
    valor_atual = conjunto_jogadores[sitPosition].valor[betAmount_ou_pot];
    if (operacao == 'adicao')
    ...
    conjunto_jogadores[sitPosition].valor[betAmount_ou_pot] = valor_final;
    //console.log("muda digito vindo de operacao")
    muda_digito(betAmount_ou_pot, sitPosition, valor_final, valor_atual);
}
...
/**
 * Função: MUDA_DIGITO
 * Objetivo: realizar animações de deslocamento de dígitos e comandos sobre os dígitos a serem rolados nos espaços betAmount e pot.
 * Como: comentários no corpo da função
...
function muda_digito(betAmount_ou_pot, sitPosition, valor_final, valor_inicial, tira_digito = false, qtde_digitos_adicionais = 0) {

    //console.log("VARIAVEIS Valor_final:" + valor_final + " - Valor_inicial:" + valor_inicial + " - tira_digito:" + tira_digito + " - qtde_digitos_adicionais:" + qtde_digitos_adicionais)

    if (parseFloat(valor_inicial).toFixed(2).replace(".", globalDecimalSeparator) > parseFloat(valor_final).toFixed(2).replace(".", globalDecimalSeparator)) {
        conjunto_jogadores[sitPosition]['direcao'][betAmount_ou_pot] = "para baixo";
    } else {
        conjunto_jogadores[sitPosition]['direcao'][betAmount_ou_pot] = "para cima";
    }
    ...
    //valor_inicial = 25692.12;
    //valor_inicial = valor_inicial.toLocaleString();
    valor_inicial = parseFloat(valor_inicial).toFixed(2).replace(".", globalDecimalSeparator).toString();
    //valor_final = 123123.12;
    //valor_final = valor_final.toLocaleString();
    valor_final = parseFloat(valor_final).toFixed(2).replace(".", globalDecimalSeparator).toString();
    //console.log("VARIAVEIS NOVAS Valor_final:" + valor_final + " - Valor_inicial:" + valor_inicial)


    if (valor_final.length < valor_inicial.length) {
        //console.log("length inicial maior que length final")
        zeros = "000.000.000.000.000.000.000,00";

        if ((tira_digito == false) && (digitos_por_jogador[sitPosition][betAmount_ou_pot]['atual'] <= (digitos_por_jogador[sitPosition][betAmount_ou_pot]['maximo'] - valor_final.length))) {
    ...
    //console.log("*********Valor_final:" + valor_final + " - Valor_inicial:" + valor_inicial)
    ...
    if (valor_final.length > valor_inicial.length) {
        ...
        condicao1 = (digitos_por_jogador[sitPosition][betAmount_ou_pot]['atual'] > (digitos_por_jogador[sitPosition][betAmount_ou_pot]['maximo']) - valor_final.length);
        ...
        direcao_deslocamento = -1;
        condicao1 = ((digitos_por_jogador[sitPosition][betAmount_ou_pot]['atual'] > (digitos_por_jogador[sitPosition][betAmount_ou_pot]['maximo']) - valor_inicial.length));
    }
    ...
    if (condicao1) {
        setTimeout(function() {
            ...
            document.getElementById(betAmount_ou_pot + "jogador" + sitPosition + "digito" + (digitos_por_jogador[sitPosition][betAmount_ou_pot]['atual'] * 1)).style.display = 'block';

            if ((valor_final.charAt(digitos_por_jogador[sitPosition][betAmount_ou_pot]['atual'] - (digitos_por_jogador[sitPosition][betAmount_ou_pot]['maximo'] - valor_final.length) - 1) == '.') &&
                (valor_inicial.charAt(digitos_por_jogador[sitPosition][betAmount_ou_pot]['atual'] - (digitos_por_jogador[sitPosition][betAmount_ou_pot]['maximo'] - valor_inicial.length) - 1) == '.')) {
                roda_digito(betAmount_ou_pot, betAmount_ou_pot + "jogador" + sitPosition + "digito" + digitos_por_jogador[sitPosition][betAmount_ou_pot]['atual'], '.', '.', conjunto_jogadores[sitPosition]['direcao'][betAmount_ou_pot], tira_digito, qtde_digitos_adicionais);
                tempo_muda_digito = 0;
            }
            ...
            else if ((valor_final.charAt(digitos_por_jogador[sitPosition][betAmount_ou_pot]['atual'] - (digitos_por_jogador[sitPosition][betAmount_ou_pot]['maximo'] - valor_final.length) - 1) == '')) {
                tempo_muda_digito = 0;
            }
            //Se o valor final for maior que o inicial e a posição for de ., solicita rotação do dígito de $ para .            
            else if ((valor_final.charAt(digitos_por_jogador[sitPosition][betAmount_ou_pot]['atual'] - (digitos_por_jogador[sitPosition][betAmount_ou_pot]['maximo'] - valor_final.length) - 1) == '.') &&
                (valor_inicial.charAt(digitos_por_jogador[sitPosition][betAmount_ou_pot]['atual'] - (digitos_por_jogador[sitPosition][betAmount_ou_pot]['maximo'] - valor_inicial.length) - 1) != '.')) {
                roda_digito(betAmount_ou_pot, betAmount_ou_pot + "jogador" + sitPosition + "digito" + digitos_por_jogador[sitPosition][betAmount_ou_pot]['atual'], '$', '.', conjunto_jogadores[sitPosition]['direcao'][betAmount_ou_pot]);
                tempo_muda_digito = 0;
            } else if ((valor_final.charAt(digitos_por_jogador[sitPosition][betAmount_ou_pot]['atual'] - (digitos_por_jogador[sitPosition][betAmount_ou_pot]['maximo'] - valor_final.length) - 1) != '.') && (valor_inicial.charAt(digitos_por_jogador[sitPosition][betAmount_ou_pot]['atual'] - (digitos_por_jogador[sitPosition][betAmount_ou_pot]['maximo'] - valor_inicial.length) - 1) == '.')) {
                document.getElementById(betAmount_ou_pot + "jogador" + sitPosition + "digito" + (digitos_por_jogador[sitPosition][betAmount_ou_pot]['atual'] * 1 - 1)).innerHTML = "<div id='" + betAmount_ou_pot + "jogador" + sitPosition + "digito" + digitos_por_jogador[sitPosition][betAmount_ou_pot]['atual'] + "teste' style='text-align:center;height:" + tamanho_digito + "px;width:" + largura_numero + "px;'>.</div>";
            ...
            //Se o valor final for maior que o inicial e a posição for de ., solicita rotação do dígito de $ para .            
                ...
                if ((valor_final.length < valor_inicial.length) && (digitos_por_jogador[sitPosition][betAmount_ou_pot]['atual'] > (digitos_por_jogador[sitPosition][betAmount_ou_pot]['maximo'] - valor_inicial.length) && digitos_por_jogador[sitPosition][betAmount_ou_pot]['atual'] <= (digitos_por_jogador[sitPosition][betAmount_ou_pot]['maximo'] - valor_final.length))) {

                    roda_digito(betAmount_ou_pot, betAmount_ou_pot + "jogador" + sitPosition + "digito" + digitos_por_jogador[sitPosition][betAmount_ou_pot]['atual'], valor_inicial.charAt(digitos_por_jogador[sitPosition][betAmount_ou_pot]['atual'] - (digitos_por_jogador[sitPosition][betAmount_ou_pot]['maximo'] - valor_inicial.length) - 1), -1, conjunto_jogadores[sitPosition]['direcao'][betAmount_ou_pot]);
                    tempo_muda_digito = 0;
                }
                //Se o tamanho do valor final for maior que o inicial e não for '.', roda o dígito para o número desejado
                else {
                    valor_inicial_digito = valor_inicial.charAt(digitos_por_jogador[sitPosition][betAmount_ou_pot]['atual'] - (digitos_por_jogador[sitPosition][betAmount_ou_pot]['maximo'] - valor_inicial.length) - 1);
                    valor_final_digito = valor_final.charAt(digitos_por_jogador[sitPosition][betAmount_ou_pot]['atual'] - (digitos_por_jogador[sitPosition][betAmount_ou_pot]['maximo'] - valor_final.length) - 1);
                    ...
                    if ((valor_final_digito < valor_inicial_digito) && (conjunto_jogadores[sitPosition]['direcao'][betAmount_ou_pot] == "para baixo"))
                        valor_final_digito = valor_final_digito * 1 + 10 * 1;

                    if ((valor_final_digito > valor_inicial_digito) && (conjunto_jogadores[sitPosition]['direcao'][betAmount_ou_pot] == "para cima")) {
                        valor_inicial_digito = valor_inicial_digito * 1 + 10 * 1;
                    }

                    document.getElementById(betAmount_ou_pot + "jogador" + sitPosition + "digito" + digitos_por_jogador[sitPosition][betAmount_ou_pot]['atual']).innerHTML = "<div id='" + betAmount_ou_pot + "jogador" + sitPosition + "digito" + digitos_por_jogador[sitPosition][betAmount_ou_pot]['atual'] + "cifrao' style='text-align:center;height:" + tamanho_digito + "px;width:" + largura_numero + "px;'>" + i * 1 + "</div>";
                    if (conjunto_jogadores[sitPosition]['direcao'][betAmount_ou_pot] == "para baixo")
                        document.getElementById(betAmount_ou_pot + "jogador" + sitPosition + "digito" + digitos_por_jogador[sitPosition][betAmount_ou_pot]['atual']).style.marginTop = '0px';

                    roda_digito(betAmount_ou_pot, betAmount_ou_pot + "jogador" + sitPosition + "digito" + digitos_por_jogador[sitPosition][betAmount_ou_pot]['atual'], valor_inicial_digito * 1, valor_final_digito, conjunto_jogadores[sitPosition]['direcao'][betAmount_ou_pot], tira_digito, qtde_digitos_adicionais);
                    tempo_muda_digito = parametros_animacao.muda_digito.velocidade;;

                }
            }
            //Desloca cursor do dígito para a esquerda
            digitos_por_jogador[sitPosition][betAmount_ou_pot]['atual']--;
            if (qtde_digitos_adicionais > 0)
                qtde_digitos_adicionais--;
            //console.log("Muda digito vindo de muda digito")
            //console.log("Muda digito vindo de muda digito esta passando Valor Final:" + valor_final + " - Valor Inicial:" + valor_inicial)
            muda_digito(betAmount_ou_pot, sitPosition, valor_final, valor_inicial, tira_digito, qtde_digitos_adicionais);
        }, tempo_muda_digito);
    } else {
        //Finalizada a rolagem, reinicializa cursor do dígito
        inicializa_digitos_max_min(sitPosition, betAmount_ou_pot);
    }
}

/**
 * Função: RODA_DIGITO
 * Objetivo: realizar animações de rolagem nos espaços betAmount e pot.
 * Como: dependendo da direção adiciona um dígito acima ou abaixo na hora que o visível iniciar a saída da zona visível
 * Observações: 
 * 
 */
function roda_digito(betAmount_ou_pot, elemento, valor_inicial, valor_final, direcao, tira_digito = false, qtde_digitos_adicionais = 0) {

    if (betAmount_ou_pot == 'betAmount') {
        tamanho_digito = texto.betAmount.digito.altura;
        largura_digito = texto.betAmount.digito.largura;
        borda = 0;
    ...
    if (valor_final == -1) {
        ...
        if (valor_final != valor_inicial - 1) {
            if (document.getElementById(elemento).style.marginTop.replace('px', '') * 1 < 0) {
                ...
                setTimeout(function() {
                    roda_digito(betAmount_ou_pot, elemento, valor_inicial, valor_final, direcao, tira_digito, qtde_digitos_adicionais)
                }, 20);
            } else {
                ...
                setTimeout(function() {
                    roda_digito(betAmount_ou_pot, elemento, valor_inicial + 1, valor_final, direcao, tira_digito, qtde_digitos_adicionais)
                }, 20);
...
/**
 * Função: VALOR_EM_betAmount
 * Objetivo: converter valor total de betAmount em grupos de betAmount por valor.
 * Como: comentários no corpo da função
 * Observações: 
 * 
 * 
...
function valor_em_betAmount(total_betAmount) {

    betAmount_temp = [];

    //Divide o valor total nos grupos de betAmount sem contar o limite.
    ...
    for (valor_betAmount_unitaria = 0; valor_betAmount_unitaria < betAmount_valores.length; valor_betAmount_unitaria++) {
        betAmount_temp[valor_betAmount_unitaria] = Math.floor(total_betAmount / betAmount_valores[valor_betAmount_unitaria].valor);
        total_betAmount = (total_betAmount % betAmount_valores[valor_betAmount_unitaria].valor);
    }

    //recontar de acordo com limite para fins de animação
    pilha_betAmount = parametros_animacao.anima_betAmount.limite_betAmount;

    for (valor_betAmount_unitaria = 0; valor_betAmount_unitaria < betAmount_temp.length; valor_betAmount_unitaria++) {
        if (betAmount_temp[valor_betAmount_unitaria] < pilha_betAmount) {
            pilha_betAmount = pilha_betAmount - betAmount_temp[valor_betAmount_unitaria];
        } else {
            betAmount_temp[valor_betAmount_unitaria] = pilha_betAmount;
            pilha_betAmount = 0;
        }
    }

    return betAmount_temp;
}

/**
 * Função: MONTA_PILHA_betAmount
 * Objetivo: constrói html com betAmount empilhadas.
 * Como: pega o array das betAmount agrupadas por valor e adiciona um DIV com a imagem correspodente àquele valor, definida nos parâmetros betAmount_valores
 * Observações: 
 * 1.O movimento de queda das betAmount é do compartimento caixa_betAmount_jogador
 * 
 */

function monta_pilha_betAmount(sitPosition, array_betAmount) {
    html = "";
    tamanho_pilha = 0;

    for (indice_valor_betAmount = 0; indice_valor_betAmount < array_betAmount.length; indice_valor_betAmount++) {
        if (array_betAmount[indice_valor_betAmount] > 0) {
            for (qtde_betAmount_por_valor = 0; qtde_betAmount_por_valor < array_betAmount[indice_valor_betAmount]; qtde_betAmount_por_valor++) {
                tamanho_pilha++;
                html = html + "<div id='betAmount" + tamanho_pilha + "jogador" + sitPosition + "' style='position:absolute;height:1vw;width:1vw;margin-left:" + (betAmount.caixa_betAmount.largura / 2 - betAmount.dimensoes.largura / 2) + "px;margin-top:" + (tamanho_pilha * parametros_animacao.anima_betAmount.distancia_entre_betAmount) + "px;z-index:" + (100 - tamanho_pilha) + "'><img src='" + betAmount_valores[indice_valor_betAmount].imagem + "' /></div>";
            }
        }
    }

    //Inclui as betAmount em um compartimento.
    document.getElementById('caixa_betAmount_jogador' + sitPosition).style.height = (tamanho_pilha * parametros_animacao.anima_betAmount.distancia_entre_betAmount + betAmount.dimensoes.altura) + 'px';
    document.getElementById('caixa_betAmount_jogador' + sitPosition).innerHTML = html;
    return tamanho_pilha;

}

/**
 * Função: ANIMA_betAmount
 * Objetivo: realiza animação de queda das betAmount.
 * Como: depois das betAmount montadas desloca as betAmount e ao chegar embaixo, realiza a subida instantânea em determinado nível
...
function anima_betAmount(sitPosition, array_betAmount, init = true, movimento = 0) {

    if (init) {
        tamanho_pilha = monta_pilha_betAmount(sitPosition, array_betAmount);
        chao = -1 * (betAmount.dimensoes.altura / 2);
        counter1 = parametros_animacao.anima_betAmount.inicio_queda;

    }

    document.getElementById('caixa_betAmount_jogador' + sitPosition).style.display = 'block';

    if (movimento < parametros_animacao.anima_betAmount2.length) {

        if (movimento % 2 == 0) //Movimento de descida
        {
            document.getElementById('caixa_betAmount_jogador' + sitPosition).style.opacity = document.getElementById('caixa_betAmount_jogador' + sitPosition).style.opacity * 1 + 0.02;

            if (counter1 < chao)
                counter1 += parametros_animacao.anima_betAmount2[movimento].velocidade;
            else {
                movimento++;
                for (posicao_pilha = 1; posicao_pilha <= tamanho_pilha; posicao_pilha++) {
                    //Quando chegar no chão, espalha as betAmount em um movimento aleatório                                    
                    aleatorio = (Math.random() - 0.5) * 1; //A variação é entre um leve deslocamento à direita e esquerda. Caso deseje aumentar a distância entre betAmount, basta aumentar o valor onde está 0.5
                    document.getElementById("betAmount" + posicao_pilha + "jogador" + sitPosition).style.marginLeft = (document.getElementById("betAmount" + posicao_pilha + "jogador" + sitPosition).style.marginLeft.replace('px', '') * 1 + aleatorio) + 'px'; //aleatorio + 'px'; //(conjunto_jogadores[sitPosition].posicao_betAmount.x * 1 + Math.random() * 10) + 'px';
                }
            }
        } else //Movimento de subida
        {
            if (counter1 >= -parametros_animacao.anima_betAmount2[movimento].posicao + chao)
                counter1 -= parametros_animacao.anima_betAmount2[movimento].velocidade;
            else
                movimento++;
        }
        document.getElementById('caixa_betAmount_jogador' + sitPosition).style.marginTop = (conjunto_jogadores[sitPosition].posicao_betAmount.y + counter1) + 'px';
        setTimeout(function() { anima_betAmount(sitPosition, array_betAmount, false, movimento) }, parametros_animacao.anima_betAmount.velocidade);
    }
 ...
 * Objetivo: inicializar jogador com a quantidade máxima de dígitos para os espaços pot e betAmount. Zerar o cursor do dígito que está sendo percorrido.
 * Como: 
 * Observações: 
 * 
 */
function inicializa_digitos_max_min(sitPosition, betAmount_ou_pot) {
    if (betAmount_ou_pot == 'todos') {
        digitos_por_jogador[sitPosition]['stack']['maximo'] = limite_digitos['stack']['maximo'];
        digitos_por_jogador[sitPosition]['stack']['atual'] = limite_digitos['stack']['atual'];
        digitos_por_jogador[sitPosition]['stack']['minimo'] = limite_digitos['stack']['minimo'];
        digitos_por_jogador[sitPosition]['betAmount']['maximo'] = limite_digitos['betAmount']['maximo'];
        digitos_por_jogador[sitPosition]['betAmount']['atual'] = limite_digitos['betAmount']['atual'];
        digitos_por_jogador[sitPosition]['betAmount']['minimo'] = limite_digitos['betAmount']['minimo'];
    } else {
        digitos_por_jogador[sitPosition][betAmount_ou_pot]['maximo'] = limite_digitos[betAmount_ou_pot]['maximo'];
        digitos_por_jogador[sitPosition][betAmount_ou_pot]['atual'] = limite_digitos[betAmount_ou_pot]['atual'];
        digitos_por_jogador[sitPosition][betAmount_ou_pot]['minimo'] = limite_digitos[betAmount_ou_pot]['minimo'];
    }
...
function inicializa_digitos_valores(sitPosition, betAmount_ou_pot) {

    total_digitos = limite_digitos[betAmount_ou_pot]['maximo'];

    if (betAmount_ou_pot == 'betAmount') {

        tamanho_digito = texto.betAmount.digito.altura;
        largura_numero = texto.betAmount.digito.largura;
        borda = 0;
    ...
    for (digito = 1; digito <= total_digitos; digito++) {
        htmlnumeros = '';
        htmlnumeros = htmlnumeros + "<div id='" + betAmount_ou_pot + "jogador" + sitPosition + "digito" + digito + "cifrao' style='text-align:center;height:" + tamanho_digito + "px;width:" + largura_numero + "px;'></div>";
        html = html + "<div id='" + betAmount_ou_pot + "jogador" + sitPosition + "digito" + digito + "' style='float:left;margin-top:" + margem_superior + "px;display:none'>" + htmlnumeros + "</div>";
    }
    ...
    document.getElementById('total' + betAmount_ou_pot + sitPosition).innerHTML = html;
}
...
function anima_opacity(elemento, aumenta_diminui, limite, contagem) {
    console.log(`anima_opacity elemento: ${elemento}, aumenta_diminui: ${aumenta_diminui}, limite: ${limite}, contagem: ${contagem}` )
    document.getElementById(elemento).style.opacity = limite;
    return
    ...
    if (aumenta_diminui == 'aumenta') {
        ...
        if (contagem <= limite) {
            ...
            setTimeout(function() {
                console.log(`anima_opacity na funcao timeout contagem < limite elemento: ${elemento}, aumenta_diminui: ${aumenta_diminui}, limite: ${limite}, contagem: ${contagem}` )
