<script>
    ...
    onMount(() => {
        ...
        if (window.api) {
            api = window.api
            // alert("loaded the api")
            // window.api.getDisplaySize()
            // let displaySize = 
            // alert(displaySize)
            // console.log(displaySize)
            // console.log("loaded the api")
            // window.api.on('screen-size', (displaySize) => {
            //     screenSize = displaySize
            //     // alert(JSON.stringify(displaySize));
            //     console.log(displaySize);
            //     // console.log(pos);
            // });
        }
    ...
    function maximizeWindow(){
        ...
        const screenSize = window.api.getDisplaySize()
        let width, height, position
        if (!isMaximized) {
            oldWidth = window.innerWidth
            oldHeight = window.innerHeight
            oldPosition = screenSize.position
            position = [0, 0]
            // window.
            width = Math.round(screenSize.height * screenSize.aspectRatio);
        ...
        api.send("set-window-size", {width: width, height: height, position: position})
...
<div class="title" on:dblclick={maximizeWindow}>
    <div class="icon">
        <svg xmlns="http://www.w3.org/2000/svg" height="15px" viewBox="0 0 809.523 128.965">
    ...
    <div class="titleText"><span>{winTitle}</span></div>
    <div class="menuButtons">
        <button on:click = {minimizeWindow}><svg xmlns="http://www.w3.org/2000/svg" width="10px" height="1px" viewBox="0 0 10 1">
            <line id="Linha_49" data-name="Linha 49" x2="10" transform="translate(0 0.5)" fill="none" stroke="#fff" stroke-width="1"/>
          </svg></button>
        <button on:click = {maximizeWindow}>
            <svg xmlns="http://www.w3.org/2000/svg" width="10px" height="10px" viewBox="0 0 10 10">
                <g id="Grupo_436" data-name="Grupo 436" transform="translate(-8079 -2512.5)">
                  <path id="União_5" data-name="União 5" d="M9.5,9.5H.5v0h0V.5h0v0h9v0h0v9h0v0Zm0,0V.5H.5v9Z" transform="translate(8079 2512.5)" fill="none" stroke="#fff" stroke-width="1"/>
                </g>
              </svg>
            
        </button>
        <button on:click = {closeWindow} class="close"><svg xmlns="http://www.w3.org/2000/svg" width="10px" height="10px" viewBox="0 0 10.707 10.707">
            <g id="Grupo_437" data-name="Grupo 437" transform="translate(-8125.146 -2512.146)">
              <path id="União_6" data-name="União 6" d="M5,5,0,10,5,5,0,0,5,5l5-5L5,5l5,5Z" transform="translate(8125.5 2512.5)" fill="none" stroke="#fff" stroke-width="1"/>
            </g>
          </svg></button>
    </div>
</div>
...
<style lang="scss">
    ...
    .close:hover {
        background-color: rgba(200,88,88,1);
    }
    .icon {
        width: 60px;
        height: 15px;
