			...
			<div class="homeTab">
				...
				<div class="homeTournamentInfo column">
					...
					<div class="tournamentInfo row">
						...
						<div class="description">
							Re-entry tournaments are tournaments where you can re-entry once you lost all your
							chips if the tournament is still in the late register period.
						</div>
					</div>
				...
				<div class="homePlayersInfo column">
					<div class="currentBlindLevel row">
						...
						<div class="description">
							Level {blindLevel} - Blinds: {blindStructure[blindLevel - 1].sb}/{blindStructure[
								blindLevel - 1
							].bb} Ante {blindStructure[blindLevel - 1].ante}
						</div>
					</div>
					<div class="nextBlindLevel row">
						<div class="type">Next Level:</div>
						<div class="description">
							Blinds: {blindStructure[blindLevel].sb}/{blindStructure[blindLevel].bb} Ante {blindStructure[
								blindLevel
							].ante} (in X min)
						</div>
					</div>
				...
				<div class="homePrizeInfo column">
					...
					<div class="playersAndPrize row">
						<div class="players column">
							<div>
								<span class="type">Entries: </span><span class="description">{totalEntries}</span>
							</div>
							<div>
								<span class="type">Players Left: </span><span class="description"
									>{playersLeft}</span
								>
							</div>
							<div>
								<span class="type">Places Paid: </span><span class="description"
									>{prizeStructure.length}</span
								>
							</div>
						</div>
				...
				<div class="blindsTable">
					<table>
						...
						{#each blindStructure as blindLevel, index}
							<tr
								><td>{index + 1}</td><td>{blindLevel.sb}/{blindLevel.bb}</td><td
									>{blindLevel.ante}</td
								><td>{blindLevelUpTime / 60000}</td></tr
							>
						{/each}
					</table>
				</div>
			...
			<div class="playersTab">
				<table>
					...
					{#each playersList as player, index}
						<tr
							><td class="rank">{index + 1}º</td><td>{player.name}</td><td>{player.stackSize}</td
							></tr
						>
					{/each}
				</table>
		...
		{:else if tabSelected === 3}
			<div class="prizePoolTab column">
				...
				<div class="players column">
					...
					<div>
						<span class="type">Players Left: </span><span class="description">{playersLeft}</span>
					</div>
					<div>
						<span class="type">Places Paid: </span><span class="description"
							>{prizeStructure.length}</span
					...
					<div>
						<span class="type">Total Prize: </span><span class="description"
							>${totalEntries * buyIn}</span
						>
					</div>
				</div>
